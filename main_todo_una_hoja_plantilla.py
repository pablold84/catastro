...
def comparar_y_resaltar(ws):
    ...
    for row in ws.iter_rows(min_row=5, max_row=max_row, min_col=col_sauce_start, max_col=ws.max_column):
        for cell in row:
            cell.border = normal_border

# Aplicar borde normal a todas las celdas en la parte de Situación Inicial
    for row in ws.iter_rows(min_row=5, max_row=max_row, min_col=col_iniciales_start, max_col=col_iniciales_end):
        for cell in row:
            cell.border = normal_border
